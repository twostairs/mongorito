...
Mongorito.events = new EventEmitter();
...
Model.prototype.toJSON = function () {
	...
	if((Array.isArray(this.hiddenAttributes) && this.hiddenAttributes.length > 0)
	 ||(typeof this.rewriteAttributes === 'object' && Object.keys(this.rewriteAttributes).length > 0) ) {
		let attributesCopy = clone(this.attributes);

		if(typeof this.rewriteAttributes === 'object') {
			Object.keys(this.rewriteAttributes).forEach(key => {
				console.log('Setting field %s to value %s', this.rewriteAttributes[key], get(attributes, key));
				set(attributesCopy, this.rewriteAttributes[key], get(attributes, key));
				unset(attributesCopy, key);
			});
		}

		if(Array.isArray(this.hiddenAttributes)) {
